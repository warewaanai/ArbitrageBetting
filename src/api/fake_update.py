    ...
    for region in regions:
        for event in events:
            if event["key"] in keys:
                new_bundles+= TheOddsAPI.fake_odds.get_odds(event["group"], region, event["key"], f'{event["title"]} ({event["description"]})')
...
#    TheOddsAPI.sports.update_active_sports()
    new_bundles = TheOddsAPI.fake_odds.get_all_bundles()

    conn_active = sqlite3.connect("active.db")
    conn_archive = sqlite3.connect("historical.db")

    #TODO: diff the old active bundles with new_bundles to archive inactive odds
